...
let rec good_rules u_rules term_rules =
	...
	| [] -> term_rules
	| (h1, h2) :: t ->
		if subset (remove_terminals h2) ((get_lhs u_rules) @ (get_lhs term_rules)) && (not (find (h1, h2) term_rules))
...
let filter_reachable g =
	...
	fst g,
	(reorder_rules rules (get_nt_rules (good_rules (set_diff rules term_rules) term_rules) term_rules))
;;
